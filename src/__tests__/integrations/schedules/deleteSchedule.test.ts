...
describe("Tests for schedules routes", () => {
  ...
  test("Must be able to delete a schedule", async () => {
    ...
    const response = await request(app).delete(`/schedules/${createSchedule.body.id}`).set("Authorization", `Bearer ${ token }`)
  ...
  test("Must prevent deleted a tokenless schedule", async () => {
    ...
    const response = await request(app).delete(`/schedules/${createSchedule.body.id}`)
  ...
  test("Must prevent deleted a schedule with invalid property id", async () => {
    ...
    const response = await request(app).delete("/schedules/05a429c8-ca25-4007-8854-25c25f734167").set("Authorization", `Bearer ${ token }`)
