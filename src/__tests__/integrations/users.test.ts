...
describe("Testing all user routes", () => {
  ...
  beforeAll(async () => {
    ...
    createdUser = await request(app).post("/api/users/signup").send(userAdm);
  ...
  test("It should prevent creating a user with an existing email", async () => {
    const response = await request(app).post("/api/users/signup").send(userAdm);
  ...
  test("Must prevent listing user properties with invalid id", async () => {
    const response = await request(app).get(
      "/api/users/05a429c8-ca25-4007-8854-25c25f734167"
    );
  ...
  test("Must prevent deactive a tokenless user", async () => {
    await request(app).patch(`/api/users/${createdUser.body.id}`);
  ...
  test("Must prevent deactivation of an already deactivated user", async () => {
    await request(app)
      .delete(`/api/users/${createdUser.body.id}`)
      .set("Authorization", `Bearer ${token}`);
  ...
  test("Must prevent deactivate user with invalid id", async () => {
    ...
    const response = await request(app)
      .delete("/api/users/05a429c8-ca25-4007-8854-25c25f734167")
      .set("Authorization", `Bearer ${token}`);
